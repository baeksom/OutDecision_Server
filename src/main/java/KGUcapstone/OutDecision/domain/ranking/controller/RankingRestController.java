...
public class RankingRestController {
    ...
    public ApiResponse<RankingResponseDTO.RankingListDTO> getAllRanking() {
        RankingResponseDTO.RankingListDTO rankingListDTO = rankingService.getTop100Rankings();
        return ApiResponse.onSuccess(rankingListDTO);
    }

    @GetMapping("/{memberId}")
    @Operation(summary = "포인트랭킹 조회(개인) API", description = "사용자의 포인트랭킹 순위를 조회합니다.")
    public ApiResponse<RankingResponseDTO.RankingDTO> getMemberRanking(@PathVariable("memberId") Long memberId) {
        RankingResponseDTO.RankingDTO memberRankingDTO = rankingService.memberRankingDTO(memberId);
